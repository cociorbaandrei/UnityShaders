Shader "Skuld/Effects/Grab Pass/Warp Type 2"
{
	Properties{
		...
		Pass
        {
			...
			*/
            ...
            half4 frag(IO vertex) : SV_Target
            {
				...
				float rotAmt = saturate(-dot(viewDirection, vertex.worldNormal)) * 3;
				float2 uv = vertex.grabPosition.xy / vertex.grabPosition.w;

				//the offset is the center of the grabpass texture.
